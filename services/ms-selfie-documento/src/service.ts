...
import { validateSelfieDocument } from "./validator";
...
export async function captureSelfieDocument(input: {
  signatoryId: string;
  documentId: string;
  imageBase64: string;
  ip?: string;
  ...
  try {
    validation = await validateSelfieDocument(input.imageBase64);
  } catch {
    validation = {
      canCapture: true,
      feedback: "validacao_indisponivel",
      validationIssues: []
    };
  }
  ...
  const selfieBase = await sharp(source)
    .resize(1280, 1920, { fit: "inside", withoutEnlargement: true })
    .ensureAlpha()
    .png()
    .toBuffer();

  const selfieMeta = await sharp(selfieBase).metadata();
  const selfieLargura = selfieMeta.width ?? 1280;
  const selfieAltura = selfieMeta.height ?? 1920;
  const margemHorizontal = 36;
  const margemSuperior = 26;
  const alturaRodape = 122;
  const canvasLargura = selfieLargura + margemHorizontal * 2;
  const canvasAltura = selfieAltura + margemSuperior * 2 + alturaRodape;
  ...
  const finalImage = await sharp({
    ...
    .composite([
      { input: selfieBase, left: margemHorizontal, top: margemSuperior },
  ...
  const prefix = normalizePrefix(env.SELFIES_PREFIX || "selfie_documento");
  const objectPath = `${prefix}/selfiedoc_${timestampFile}_${randomUUID()}.png`;

  await minioClient.putObject(env.BUCKET_SELFIES, objectPath, finalImage, finalImage.length, {
  ...
  const typedValidation = validation as {
    faceStatus?: { detected?: boolean };
    docStatus?: { detected?: boolean };
  };

  const { rows } = await db.query(
    `INSERT INTO selfies_documentos (document_id, signatory_id, image_path, file_hash, face_detected, document_detected, ip, user_agent, latitude, longitude)
    ...
    [
      ...
      objectPath,
      hash,
      Boolean(typedValidation.faceStatus?.detected),
      Boolean(typedValidation.docStatus?.detected),
      effectiveIp ?? null,
